...
export const cursor = function (gsapContext) {
  ...
  const DISABLE = '[data-ix-cursor="disable"]';
  //options
  ...
  //add hover class to the cursor when it hover over links
  ...
  cursorDisable();
  // // Interaction when a click event occurs
  ...
  const cursorMove = function () {
    ...
    //innner timelines
    let innerX = gsap.quickTo(cursorInner, 'x', { duration: INNER_DELAY, ease: 'none' });
    let innerY = gsap.quickTo(cursorInner, 'y', { duration: INNER_DELAY, ease: 'none' });
    //outer timelines
